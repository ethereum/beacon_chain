from beacon_chain.bls import G1, G2, hash_to_G2, compress_G1, compress_G2, \
    decompress_G1, decompress_G2, normalize, multiply, \
    sign, privtopub, aggregate_sigs, aggregate_pubs, verify

from beacon_chain.simpleserialize import serialize, deserialize, eq

from beacon_chain.full_pos import ActiveState, CheckpointRecord
...
def test_all():
    ...
    assert eq(s, ds)
    s = ActiveState(checkpoints=[CheckpointRecord(shard_id=42, checkpoint_hash=b'\x55'*32, voter_bitmask=b'31337dawg')],
